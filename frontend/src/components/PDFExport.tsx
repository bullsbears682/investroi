...
const PDFExport: React.FC<PDFExportProps> = ({ calculationData }) => {
  ...
  const generateSimplePDF = () => {
    ...
    doc.text('ROI Investment Report', 105, 40, { align: 'center' });

    // Main content area with app-style glass effect
    ...
    doc.text(`${roi.toFixed(2)}% ROI`, 105, 80, { align: 'center' });
    
    doc.setFontSize(12);
    doc.setTextColor(255, 255, 255);
    doc.text(`on $${calculationData.total_investment?.toLocaleString() || 'N/A'} investment`, 105, 90, { align: 'center' });

    // Summary cards (app-style glass cards)
    const summaryData = [
    ...
    // Performance indicator (app-style)
  ...
  const generateDetailedPDF = () => {
    ...
    // Executive Summary Card (app-style glass)
    doc.setFillColor(255, 255, 255, 0.1);
    doc.rect(20, 70, 170, 35, 'F');
    doc.setDrawColor(255, 255, 255, 0.2);
    doc.rect(20, 70, 170, 35, 'S');
    ...
    doc.setFontSize(16);
    doc.setTextColor(255, 255, 255);
    doc.text('Executive Summary', 25, 85);
    
    doc.setFontSize(20);
    doc.setTextColor(roiColor[0], roiColor[1], roiColor[2]);
    doc.text(`${roi.toFixed(2)}% ROI`, 105, 85, { align: 'center' });
    
    doc.setFontSize(11);
    doc.setTextColor(255, 255, 255, 0.8);
    doc.text(`Total Investment: $${calculationData.total_investment?.toLocaleString() || 'N/A'}`, 105, 95, { align: 'center' });

    // Investment Breakdown Section (app-style)
    doc.setFontSize(16);
    doc.setTextColor(255, 255, 255);
    doc.text('💰 Investment Breakdown', 20, 120);
    ...
    // ROI Performance Section (app-style)
    doc.setFontSize(16);
    doc.setTextColor(255, 255, 255);
    ...
    // Business Information Section (app-style)
    doc.setFontSize(16);
    doc.setTextColor(255, 255, 255);
    ...
    // Risk Assessment (app-style)
    doc.setFontSize(16);
    doc.setTextColor(255, 255, 255);
    doc.text('⚠️ Risk Assessment', 20, 320);
    ...
    doc.text(riskLevel, 20, 330);

    // App-style footer
    doc.setFillColor(255, 255, 255, 0.1);
    doc.rect(0, 350, 210, 30, 'F');
    
    doc.setFontSize(9);
    doc.setTextColor(255, 255, 255);
    doc.text('Generated by InvestWise Pro ROI Calculator', 105, 360, { align: 'center' });
    doc.text('Professional Investment Analysis Tool', 105, 367, { align: 'center' });
  ...
  const generateExecutivePDF = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(147, 51, 234);
    doc.text('Executive Summary', 105, 50, { align: 'center' });
    
    doc.setFontSize(12);
    doc.setTextColor(255, 255, 255, 0.8);
    doc.text(`Generated: ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}`, 105, 65, { align: 'center' });

    // Key Performance Indicators Box (app-style glass)
    doc.setFillColor(255, 255, 255, 0.1);
    doc.rect(20, 80, 170, 45, 'F');
    doc.setDrawColor(255, 255, 255, 0.2);
    doc.rect(20, 80, 170, 45, 'S');
    ...
    doc.setFontSize(18);
    doc.setTextColor(255, 255, 255);
    doc.text('🎯 Key Performance Indicators', 105, 95, { align: 'center' });
    
    doc.setFontSize(26);
    doc.setTextColor(roiColor[0], roiColor[1], roiColor[2]);
    doc.text(`${roi.toFixed(2)}% ROI`, 105, 110, { align: 'center' });
    
    doc.setFontSize(12);
    doc.setTextColor(255, 255, 255, 0.8);
    doc.text(`on $${calculationData.total_investment?.toLocaleString() || 'N/A'} investment`, 105, 120, { align: 'center' });

    // Executive Summary Table (app-style)
    doc.setFontSize(16);
    doc.setTextColor(255, 255, 255);
    doc.text('📊 Investment Overview', 20, 140);
    ...
    doc.setFontSize(16);
    doc.setTextColor(255, 255, 255);
    doc.text('📋 Investment Assessment', 20, 225);
    ...
    doc.text(`Confidence Level: ${confidence}`, 20, 245);
    
    doc.setFontSize(10);
    doc.setTextColor(255, 255, 255, 0.8);
    doc.text(`Based on ${roi.toFixed(2)}% ROI analysis`, 20, 255);

    // Market Analysis Section (app-style)
    doc.setFontSize(16);
    doc.setTextColor(255, 255, 255);
    doc.text('📈 Market Analysis', 20, 275);
    ...
    marketInsights.forEach((insight, index) => {
      doc.text(insight, 20, 285 + (index * 8));
    });
    ...
    doc.text('For Executive Decision Making', 105, 344, { align: 'center' });
