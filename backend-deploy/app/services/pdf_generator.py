...
class PDFGeneratorService:
    ...
    def generate_simple_report(self, calculation_data: Dict[str, Any], white_label_config: Dict[str, Any] = None) -> str:
        """Generate a simple ROI report PDF with optional white label branding"""
        ...
        try:
            ...
            story.extend(self._create_simple_header(calculation_data, white_label_config))
            story.extend(self._create_simple_summary(calculation_data))
            story.extend(self._create_simple_details(calculation_data))
            
            # Add white label footer if configured
            if white_label_config:
                story.extend(self._create_white_label_footer(white_label_config))
    ...
    def _create_simple_header(self, data: Dict[str, Any], white_label_config: Dict[str, Any] = None) -> List:
        """Create the report header with optional white label branding"""
        elements = []
        
        # White label company header or default title
        if white_label_config:
            # Company name and branding
            company_title = Paragraph(
                white_label_config.get('pdf_header_text', f"{white_label_config.get('company_name')} - ROI Investment Report"),
            ...
            contact_info = []
            if white_label_config.get('website'):
                contact_info.append(f"Website: {white_label_config['website']}")
            if white_label_config.get('support_email'):
                contact_info.append(f"Email: {white_label_config['support_email']}")
            if white_label_config.get('phone_number'):
                contact_info.append(f"Phone: {white_label_config['phone_number']}")
    ...
    def _create_white_label_footer(self, white_label_config: Dict[str, Any]) -> List:
        """Create white label footer section"""
        elements = []
        
        elements.append(Spacer(1, 30))
        
        # Footer text
        footer_text = white_label_config.get('pdf_footer_text', f"Generated by {white_label_config.get('company_name')}")
        ...
        if white_label_config.get('company_address'):
            address_paragraph = Paragraph(
                white_label_config['company_address'],
                ParagraphStyle(
                    'Address',
                    parent=self.styles['Normal'],
                    fontSize=9,
        ...
        disclaimer = Paragraph(
            "<i>This report is for informational purposes only and does not constitute financial advice. "
